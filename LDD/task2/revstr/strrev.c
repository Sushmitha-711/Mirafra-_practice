...
static ssize_t my_write(struct file *file,const char *buff,size_t len,loff_t *offset)
{
	...
	printk("buff:%s\tkernel_buff:%s\n",buff,kernel_buffer);
strcpy(kernel_buffer,strrev(kernel_buffer));
	printk("done write\n");
	return len;
}
char *strrev(char kernel_buffer[])
{
	int i=0,j;
	for(;kernel_buffer[i]!=0;i++);
	i--;
	for(j=0;j<=i;i--,j++)
	{
...
static int __init start(void)
{
	 ...
	 */
	if(alloc_chrdev_region(&cdev,0,1,"revstr")<0)
	{
